...
func RankActions(decision *app.DecisionContext, hidden HiddenCards) []ActionEvaluation {
	...
	for _, action := range decision.LegalActions {
		features := actionFeatures(action, decision)
...
func actionFeatures(action domain.Action, decision *app.DecisionContext) []FeatureContribution {
	switch action.Kind {
	case domain.ActionKindDefend:
		return defendActionFeatures(action, decision)
	case domain.ActionKindTake:
		return takeActionFeatures(decision)
	case domain.ActionKindAttack, domain.ActionKindThrowIn:
		return attackActionFeatures(action, decision)
...
func defendActionFeatures(action domain.Action, decision *app.DecisionContext) []FeatureContribution {
	score := Score(140)
...
func attackActionFeatures(action domain.Action, decision *app.DecisionContext) []FeatureContribution {
	kindScore := Score(80)
